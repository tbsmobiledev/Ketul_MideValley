...
class GetChannelsSectionAPI(context: Context, responseListener: ResponseListener) {
    ...
    private var mChannelsBean : ChannelSectionBean = ChannelSectionBean()
     ...
     */
    fun execute() {
        mJsonObjectRequest = object : JsonObjectRequest(Method.GET,Config.API_CHANNELS_SECTION + mParams
            , mJsonObject ,
        ...
        AppSingleton.getInstance(mContext).addToRequestQueue(mJsonObjectRequest, Config.TAG_CHANNELS_SECTION)
    }
    ...
    private fun parse(response: JSONObject?) {
        ...
        try {
            ...
            val gson = Gson()
            val channelsBean1 = gson.fromJson(response.toString(), ChannelSectionBean::class.java)
    ...
    private fun doCallBack(code: Int, mesg: String, objects: Any) {
        try {
            if (code == Config.API_SUCCESS) {
                mResponseListener.onResponse(Config.TAG_CHANNELS_SECTION, Config.API_SUCCESS, objects)
            } else if (code > Config.API_SUCCESS) {
                mResponseListener.onResponse(Config.TAG_CHANNELS_SECTION, Config.API_FAIL, mesg)
            } else if (code < Config.API_SUCCESS) {
                mResponseListener.onResponse(Config.TAG_CHANNELS_SECTION, Config.API_FAIL, mesg)
            }
        } catch (e: Exception) {
